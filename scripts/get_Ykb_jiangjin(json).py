...
import scripts.utilGetUrlPage as getPage
import scripts.utilAsyncGetHtml as getHtml
import scripts.utilParseHtml as parseHtml
import scripts.utilDict as utilDict
import scripts.utilFile as utilFile
...
def getXzqlqdUrl(filename,pageNo):
    ...
    # 获取市级行政权力清单
    data = '{"txnCommCom":{"txnIttChnlId":"C0071234567890987654321","txnIttChnlCgyCode":"BC01C101","tRecInPage":10,"tPageJump":'+str(pageNo)+'},"txnBodyCom":{"name":"","strLevel":1,"minnum":"","regnCode":"500116","addrLvlCd":"3","type":"XZSP","fwlb":""}}'
    #测试南川公安局行政确认
    ...
    for list in list2:
        ...
        #查看是否有下级目录
        data = '{"txnCommCom":{"txnIttChnlId":"C0071234567890987654321","txnIttChnlCgyCode":"BC01C101","tRecInPage":100,"tPageJump":1},"txnBodyCom":{"matterId":"'+groupId+'","strLevel":2,"regnCode":"500116","addrLvlCd":"3","exeLevel":"4"}}'
        ...
        if len(listz)>=1:
            for l2 in listz:
                ...
                title = l2['name']
                id = l2['id']
                href = 'https://zwykb.cq.gov.cn/qxzz/jjq/bszn/?id=' + id + '&parentPage=7'
                row2 = [title, id, href]
                rows.append(row2)
                sonList = l2['sonList']
                for son in sonList:
                    title3=son['name']
                    id3=son['id']
                    href3='https://zwykb.cq.gov.cn/qxzz/jjq/bszn/?id='+id3+'&parentPage=7'
                    if title==title3:
                        rows.remove(row2)
                    rows.append([title3,id3,href3])
                print(rows)
                utilFile.tocsv(filename, rows)
...
def getGgfwUrl(filename,pageNo):
    print('第' + str(pageNo) + '页')
    # 获取公共服务清单
    data = '{"txnCommCom":{"txnIttChnlId":"C0071234567890987654321","txnIttChnlCgyCode":"BC01C101","tRecInPage":10,"tPageJump":'+str(pageNo)+'},"txnBodyCom":{"name":"","strLevel":1,"minnum":"","regnCode":"500116","addrLvlCd":"3","type":"20","fwlb":""}}'
    res = requests.post('https://ykbapp.cq.gov.cn:8082/gml/web10022', headers=headers, data=data).json()
    list2 = json.loads(res['C-Response-Body'])['lIST2']
    for list in list2:
        matterId = list['matterId']
        #查看是否有下级目录
        data = '{"txnCommCom":{"txnIttChnlId":"C0071234567890987654321","txnIttChnlCgyCode":"BC01C101","tRecInPage":100,"tPageJump":1},"txnBodyCom":{"matterId":"'+matterId+'","strLevel":2,"regnCode":"500116","addrLvlCd":"3"}}'
...
if __name__ == '__main__':
    # 1 获取所需爬取页面总页数
    url='https://zwykb.cq.gov.cn/qxzz/jjq/fwqd/xzqlqd/'
    xpath='/html/body/div[4]/div[5]/div[6]/a[6]'
    tptalPage = getPage.geturl(url,xpath)
    print(tptalPage)
    #直接爬取链接
    # for i in range(1,tptalPage+1):#435页
    #     getXzqlqdUrl('渝快办江津url.csv', i)

    url = 'https://zwykb.cq.gov.cn/qxzz/jjq/fwqd/ggfwqd/'
    xpath = '/html/body/div[4]/div[5]/div[6]/a[6]'
    tptalPage = getPage.geturl(url, xpath)
    print(tptalPage)

    for i in range(1, 30 + 1):#32页
        getGgfwUrl('渝快办江津url.csv', i)
    #
    # # 读取存储url csv到数组中
    # utilFile.readurl('渝快办江津url.csv')
    # # 定义多进程共享变量
    # manager = Manager()
    # rows = manager.list()
    # main_get_html()
    # main_parse_html(rows)
    # utilFile.tocsv('渝快办江津body.csv', rows)
