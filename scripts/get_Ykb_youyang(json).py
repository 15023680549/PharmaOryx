    ...
    for list in list2:
        ...
        if len(listz)>=1:
            for l2 in listz:
                ...
                title = l2['name']
                id = l2['id']
                href = 'https://zwykb.cq.gov.cn/qxzz/yyxxx/bszn/?id=' + id + '&parentPage=7'
                row2 = [title, id, href]
                rows.append(row2)
                sonList = l2['sonList']
                for son in sonList:
                    title3=son['name']
                    id3=son['id']
                    href3='https://zwykb.cq.gov.cn/qxzz/yyxxx/bszn/?id='+id3+'&parentPage=7'
                    if title==title3:
                        rows.remove(row2)
...
if __name__ == '__main__':
    ...
    # 读取存储url csv到数组中
    readurl('渝快办酉阳url.csv')
    # 定义多进程共享变量
    manager = Manager()
    rows = manager.list()
    main_get_html()
    main_parse_html(rows)
    tocsv('渝快办酉阳body.csv', rows)
